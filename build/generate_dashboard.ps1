...
# Адреса встроенных приложений Apps Script. Держим их в секретах, а не в коде:
# репозиторий публичный, а приложения развёрнуты «для всех» - по прямой ссылке
# любой открыл бы доску задач или форму входа в реестр. В дашборд они попадают
# внутрь зашифрованной начинки, то есть доступны только по паролю дашборда.
$plannerUrl = Get-OpuSecret 'OPU_PLANNER_URL' 'opu_planner_url.txt'
if ([string]::IsNullOrWhiteSpace($plannerUrl)) { throw "Не задан адрес планировщика: переменная OPU_PLANNER_URL или файл opu_planner_url.txt" }
# Реестр - необязательный: если адреса нет, вкладка просто не появится.
$reestrUrl = Get-OpuSecret 'OPU_REESTR_URL' 'opu_reestr_url.txt'
...
$sb.AppendLine('*{box-sizing:border-box;margin:0;padding:0}body{font-family:"Segoe UI",Arial,sans-serif;background:radial-gradient(circle at 18% 12%,#2563eb 0,transparent 35%),radial-gradient(circle at 84% 88%,#1e40af 0,transparent 38%),linear-gradient(135deg,#0f172a,#172554 52%,#0c4a6e);background-attachment:fixed;color:#1e293b;padding:20px;font-size:14px}') | Out-Null
...
$sb.AppendLine('.more-tabs{position:relative}.more-tabs-menu{display:none;position:absolute;z-index:50;top:calc(100% + 6px);right:0;min-width:210px;padding:5px;background:#fff;border:1px solid #cbd5e1;border-radius:8px;box-shadow:0 8px 22px rgba(15,23,42,.16)}.more-tabs-menu.open{display:block}.more-tabs-menu .tab-btn{width:100%;justify-content:flex-start;white-space:nowrap;background:#fff;border-radius:5px}.more-tabs-menu .tab-btn:hover{background:#f1f5f9}.more-tabs-menu .tab-btn.active{background:#1e3a5f;color:#fff}') | Out-Null
...
$attPlanHint = if ($plannerAvailable) { "Нажмите, чтобы открыть список" } else { "Планировщик не найден в выгрузке" }
...
# Адреса подставляются из секретов (см. начало файла), а не зашиты в код.
# Append (а не AppendLine с "...") - чтобы PowerShell не пытался разбирать $ в ссылке.
$sb.Append('<div id="tab-plnr" style="display:none"><iframe id="plnr-frame" title="Планировщик ОПУ" data-src="') | Out-Null
$sb.Append($plannerUrl) | Out-Null
$sb.AppendLine('" style="width:100%;height:calc(100vh - 160px);min-height:560px;border:1px solid #e2e8f0;border-radius:10px;background:#fff;display:block"></iframe></div>') | Out-Null

if (-not [string]::IsNullOrWhiteSpace($reestrUrl)) {
    $sb.Append('<div id="tab-reestr" style="display:none"><iframe id="reestr-frame" title="Реестр ОПУ" data-src="') | Out-Null
    $sb.Append($reestrUrl) | Out-Null
    $sb.AppendLine('" style="width:100%;height:calc(100vh - 160px);min-height:560px;border:1px solid #e2e8f0;border-radius:10px;background:#fff;display:block"></iframe></div>') | Out-Null
}
$sb.AppendLine(@'
...
'@) | Out-Null
$sb.AppendLine('function switchTab(t){["dash","rem","ai","plan","plnr","proto","mc","sver","reestr"].forEach(function(id){var el=document.getElementById("tab-"+id);if(el)el.style.display=t===id?"block":"none";var btn=document.getElementById("tab-btn-"+id);if(btn)btn.classList.toggle("active",t===id);});if(t==="plan")planRender();if(t==="plnr"){var f=document.getElementById("plnr-frame");if(f&&!f.src&&f.getAttribute("data-src"))f.src=f.getAttribute("data-src");}if(t==="reestr"){var rf=document.getElementById("reestr-frame");if(rf&&!rf.src&&rf.getAttribute("data-src"))rf.src=rf.getAttribute("data-src");}if(t==="proto")protoInit();if(t==="mc"){mcInit();if(!window._mcRan){window._mcRan=true;mcRunSim();}}if(t==="sver")sverRender();}') | Out-Null
$sb.AppendLine('function setupMoreTabs(){var nav=document.querySelector(".tab-nav");if(!nav||document.getElementById("tab-btn-more"))return;var ids=["plan","proto","sver"],holder=document.createElement("div"),more=document.createElement("button"),menu=document.createElement("div");holder.className="more-tabs";more.className="tab-btn";more.id="tab-btn-more";more.type="button";more.innerHTML="Ещё &#9662;";more.setAttribute("aria-expanded","false");menu.className="more-tabs-menu";ids.forEach(function(id){var btn=document.getElementById("tab-btn-"+id);if(btn)menu.appendChild(btn);});more.onclick=function(e){e.stopPropagation();var open=menu.classList.toggle("open");more.setAttribute("aria-expanded",open?"true":"false");};holder.appendChild(more);holder.appendChild(menu);nav.appendChild(holder);document.addEventListener("click",function(e){if(!holder.contains(e.target)){menu.classList.remove("open");more.setAttribute("aria-expanded","false");}});var baseSwitch=switchTab;switchTab=function(t){baseSwitch(t);var inMore=ids.indexOf(t)!==-1;more.classList.toggle("active",inMore);menu.classList.remove("open");more.setAttribute("aria-expanded","false");};}') | Out-Null
$sb.AppendLine('document.addEventListener("DOMContentLoaded",setupMoreTabs);') | Out-Null
